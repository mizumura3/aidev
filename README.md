...
| `base` | string | ブランチ作成元 |
| `language` | `ja` \| `en` | コメント/エージェント出力の言語 |
| `skip` | string[] | スキップする工程（下記参照） |
| `backend` | string | 使用するバックエンドランナー |
| `model` | string | バックエンドで使用するモデル |

同じパラメータは `.aidev.yml`（`aidev init` で生成）にも指定できる。`language` のデフォルトは `ja`。
`language` は Investigation/Review コメント、および planner/reviewer の自然言語出力に適用される。
